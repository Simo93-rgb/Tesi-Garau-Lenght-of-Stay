...
% ── NUOVA SLIDE 4: I 3 Approcci di Embedding ─────────────────
\section{Metodologia}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.33\textwidth}
      \begin{block}{2. Binning Temporale}
        \small
        Discretizzazione \textit{data-driven} del tempo. Inserimento di token speciali derivati dalla distribuzione empirica:
        \begin{itemize}
            \item \texttt{\_DELTA\_SHORT\_}
            \item \texttt{\_DELTA\_MEDIUM\_} (1-7gg)
        \end{itemize}
      \end{block}
    \end{column}
...
% ── MODIFICA VECCHIA SLIDE 4 -> DIVENTA SLIDE 5 ──────────────
\begin{frame}{L'Approccio Vincente: Il Paradigma ``Storytelling''}
  \textbf{L'Intuizione:} tradurre la traccia in un \textbf{testo narrativo} massimizza 
  ...
  \begin{itemize}
      ...
      \item \textbf{Eventi Simultanei:} Aggregazione dinamica in \textit{bullet points} testuali per evitare ridondanza e non sovraccaricare l'Attention di BERT.
  \end{itemize}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      ...
      \begin{itemize}
        ...
        \item Setup (5-Fold CV): \texttt{lr}=$5\text{e-}5$, batch=22, early stopping epoca 10
      \end{itemize}
...
\begin{frame}{Il Problema della Black-Box in Sanità}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      ...
      \begin{itemize}
        \item Grad-CAM $\to$ solo per CNN/immagini
        \item Vanilla Gradient $\to$ \textit{gradient shattering}
        \item SmoothGrad / Blur IG $\to$ no embedding discreti
      \end{itemize}

      \vspace{0.2em}
      \textbf{Soluzione adotatta:} \alert{Integrated Gradients (IG)}\\
      assiomaticamente verificabili per sequenze testuali.
    \end{column}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.42\textwidth}
      ...
      \begin{exampleblock}{Ottimizzazione Hardware}
        ...
        Inoltre, il \textbf{90\%} dei campioni converge ad\\
        appena $m{=}1500$ steps di integrazione.
      \end{exampleblock}
    \end{column}
  \end{columns}
\end{frame}
